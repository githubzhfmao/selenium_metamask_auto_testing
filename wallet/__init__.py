...
def getMnemonic(file):
    # 用户助记词路径，以xlsx格式保存，该路径由用户提供,在config.ini中配置
    # file = global_config.get('path', 'wallet_path').strip()
    mnemonic_list = Excel(file).getColValues(1)
...
def getAddress(filename):
    if len(filename) == 0:
        ...
        return
    # 用户地址路径，以xlsx格式保存
    file = '/Users/paul/Desktop/testzksync/' + filename
    address_list = Excel(file).getColValues(1)
    return address_list
...
def getSeedPhraseV2(address,filename):
    input_address = address
    wallet = getWallet(filename)
    address_list = wallet[0]
